...
export const TextInput: FC<{
  ...
  return (
    <Input
      ...
      onChange={handleChange}
      onBlur={handleBlur}
      value={value ? value : form.values[field.name]}
    />
  );
};
...
export const PasswordInput: FC<{
  ...
  return (
    <>
              ...
              {levelPassword.map((item) => (
                <span
                  className={`input__label--level-item ${
                    item.level <= validate.passwordStrength(fieldVal) &&
                    addClassLevel
                  }`}
                  key={item.level}
                />
              ...
              <span className={`input__label--level-title ${addClassLevel}`}>
                {t(
                  levelPassword.filter(
                    (item) => item.level === validate.passwordStrength(fieldVal)
                  )[0]?.text
                )}
              </span>
            </div>
      ...
      <Password {...field} {...props} autoComplete="off" />
    </>
  );
};
